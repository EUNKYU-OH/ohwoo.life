...
const AccountModal: React.FC<AccountModalProps> = ({ selectedAccounts, onClose }) => {
  ...
  return (
    // ⭐️ 모달 배경: bg-white와 opacity-100을 사용하여 전체 화면을 불투명한 흰색으로 덮음
    <div 
      className="fixed inset-0 bg-white opacity-100 flex items-center justify-center z-[9999] p-4"
      onClick={(e) => {
        // 배경 클릭 시 모달 닫기 (배경이 흰색이지만 이벤트를 그대로 사용)
        if (e.target === e.currentTarget) {
          onClose();
        }
      }}
    >
      {/* 모달 본체: 팝업 콘텐츠 영역. 흰색 배경 위에 그림자만 추가 */}
      <div className={`
